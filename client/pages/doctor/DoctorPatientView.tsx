...
export default function DoctorPatientView() {
  ...
  const loadWP = (): WeeklyPlan | null => {
    try {
      const raw = localStorage.getItem(wpKey);
      return raw ? (JSON.parse(raw) as WeeklyPlan) : null;
    } catch {
      return null;
    }
  };
  const saveWP = (wp: WeeklyPlan) =>
    localStorage.setItem(wpKey, JSON.stringify(wp));
  ...
  const generate7 = () => {
    ...
    const days: DayPlan[] = Array.from({ length: 7 }).map((_, i) => {
      ...
      const addSnack: Meal = {
        ...
        type: "Snack",
        dosha: (req?.patientDosha || "Tridoshic") as any,
        rasa: "Madhura",
        ...calcMacros(180),
      };
      const withTypes: Meal[] = base.meals.map((m) => {
        ...
        const rasaProp =
          (m.properties || []).find((p) => p.startsWith("Rasa:")) ||
          "Rasa: Madhura";
  ...
  const stats = useMemo(() => {
    ...
    const avgCal = perDay.length
      ...
      : 0;
    const avgWater = perDay.length
      ? Math.round(perDay.reduce((s, x) => s + x.water, 0) / perDay.length)
      : 0;
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <div className="text-xl font-semibold">
            {req.patientName || `Patient ${req.userId}`}
          </div>
          {weekly ? (
            <div className="text-sm text-muted-foreground">
              7-Day Diet Plan • Avg {stats.avgCal} kcal • Avg {stats.avgWater}{" "}
              ml water
            </div>
          ) : (
            <div className="text-sm text-muted-foreground">
              No analysis yet • Generate a 7-day plan
            </div>
          )}
        </div>
        <div className="flex gap-2">
          ...
          <Button
            variant="outline"
            onClick={() =>
              navigate(`/doctor/generator/recipes?pid=${req.userId}`)
            }
          >
            Recipe Generator
          </Button>
          <Button
            onClick={() =>
              navigate("/doctor/generator/diet", { state: { pid: req.userId } })
            }
          >
            Generate Diet Plan
          </Button>
          ...
          {profile ? (
            ...
            <div className="space-y-3">
              ...
              <div className="mt-3 rounded-md bg-muted p-3 text-sm text-muted-foreground">
                Complete patient profile not available. Add medical details to
          ...
          <Card>
            ...
            <CardContent>
              ...
              <div className="mt-2 text-sm text-muted-foreground">
                Averages: {stats.avgCal} kcal/day • {stats.avgWater} ml
                water/day
              </div>
            </CardContent>
          </Card>
                              ...
                              <TooltipContent>
                                <div className="max-w-xs text-xs">
                                  ...
                                  {m.properties && m.properties.length > 0 && (
                                    <div>Props: {m.properties.join(", ")}</div>
                                  )}
                                </div>
                              </TooltipContent>
                            </Tooltip>
          ...
          <CardContent>
            <div className="rounded-md bg-destructive/10 p-3 text-sm">
              ...
              <div className="text-muted-foreground">
                No analysis available yet. Generate a 7-day diet plan and begin
