...
type ChatMessage = { role: "user" | "bot"; text?: string; image?: string; ts: number };

export const ChatWidget: React.FC<{ mode?: "floating" | "panel" }> = ({ mode = "floating" }) => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: "bot", text: "Hi! I'm your Ayurvedic assistant. Tell me what you ate or ask for tips.", ts: Date.now() - 60000 },
    { role: "user", text: "Any quick tip for digestion?", ts: Date.now() - 45000 },
    { role: "bot", text: "Favor warm, cooked meals. Sip ginger-cumin tea after meals.", ts: Date.now() - 42000 },
  ]);
  ...
  const startCamera = async () => {
    try {
      ...
      setMessages((m)=>m.concat({ role:"bot", text:"Camera access denied. You can still upload an image.", ts: Date.now() }));
    }
  };
  ...
  const capturePhoto = () => {
    ...
    setMessages((m)=>m.concat({ role:"user", image: dataUrl, ts: Date.now() }));
    setTimeout(()=> setMessages((m)=>m.concat({ role:"bot", text:"Got it! I detect whole grains and fresh produce. Want a quick calorie estimate?", ts: Date.now() })), 500);
  };
  ...
  const Header = (
    <div className={cn("flex items-center justify-between border-b px-3 py-2", isFloating && "cursor-move")} onMouseDown={onMouseDownHeader}>
      ...
      {messages.map((m, i) => (
        <div key={i} className={cn("flex", m.role === "user" ? "justify-end" : "justify-start")}>
          <div className="max-w-[80%]">
            {m.image ? (
              <img src={m.image} alt="captured" className={cn("rounded-md", m.role === "user" ? "border-2 border-primary" : "border")} />
            ) : (
    ...
    <div className="border-t p-2">
      ...
      {cameraOn && (
        <div className="mb-2 rounded-md border p-2">
          <video ref={videoRef} className="h-40 w-full rounded-md bg-black/20 object-contain" />
          <div className="mt-2 flex justify-end gap-2">
            <Button size="sm" variant="secondary" onClick={capturePhoto}>Capture</Button>
          </div>
        </div>
      )}
